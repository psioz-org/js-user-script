...
(function() {
  ...
  if (true) { //Ask popup: confirm('Remove finished story')
    function removeNondev() {
      for (let xs=document.querySelectorAll('span.ghx-info > span'),i=0,x,s,parent;(x=xs[i]);i++) {
        parent=x.parentNode.parentNode.parentNode.parentNode
        if (parent.querySelectorAll('.ghx-column .ghx-subtask-group').length>0) {
          var show=false
